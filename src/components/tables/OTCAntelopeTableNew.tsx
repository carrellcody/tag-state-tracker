...
export function OTCAntelopeTableNew() {
  const { data, loading, error } = useCsvData(`/data/antOTC25.csv?v=${CSV_VERSION}`);
  const { favorites, toggleFavorite: toggleFavoriteRaw, clearAllFavorites } = useFavorites('otc_antelope_new');
  ...
  const [unitSearch, setUnitSearch] = usePersistedState('otcAntelopeNew_unitSearch', '');
  const [showFavoritesOnly, setShowFavoritesOnly] = usePersistedState('otcAntelopeNew_showFavoritesOnly', false);
  const [selectedSeason, setSelectedSeason] = usePersistedState('otcAntelopeNew_selectedSeason', '');
  const [minBuckDoe, setMinBuckDoe] = usePersistedState('otcAntelopeNew_minBuckDoe', '');
  const [minPublicLand, setMinPublicLand] = usePersistedState('otcAntelopeNew_minPublicLand', '');
  ...
  const filteredData = useMemo(() => {
    return data.filter((row: any) => {
      ...
      if (minBuckDoe) {
        const bv = parseFloat(String(row['Buck/Doe Ratio'] || '').replace(/[, ]/g, ''));
  ...
  const visibleColumns = ['Unit', 'Acres', 'Acres Public', 'DAU', 'Population', 'DAUAnimalDensityNorm', 'Buck/Doe Ratio', 'DAUBuckDensity', 'Success_DAU'];
  ...
  const headerLabels: Record<string, string> = {
    ...
    'DAUAnimalDensityNorm': 'Normalized Pronghorn Density (0-1)',
  ...
  const groupedColumns = ['Population', 'DAUAnimalDensityNorm', 'Buck/Doe Ratio', 'DAUBuckDensity', 'Success_DAU'];
            ...
            <div className="space-y-1">
              {otcSeasons.map(season => (
                <div key={season} className="flex items-center space-x-2">
                  <input
                    type="radio"
                    id={`otcAntelopeNew-season-${season}`}
                    name="otcAntelopeSeason"
                    checked={selectedSeason === season}
                    onChange={() => setSelectedSeason(season)}
                    className="rounded"
                  />
                  <Label htmlFor={`otcAntelopeNew-season-${season}`} className="cursor-pointer text-sm">{season}</Label>
                </div>
              ))}
            </div>
          </div>
