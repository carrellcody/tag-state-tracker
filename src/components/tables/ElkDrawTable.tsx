...
function getHybridCellValue(row: any): string {
  ...
  return String(hybridKey ? row[hybridKey] : "")
    .replace(/[\u00A0]/g, " ")
    .trim();
}
...
export function ElkDrawTable() {
  ...
  const [hunterClassManuallyChanged, setHunterClassManuallyChanged] = usePersistedState(
    "elkDraw_hunterClassManuallyChanged",
    false,
  );
  ...
  const [showNewTags, setShowNewTags] = usePersistedState("elkDraw_showNewTags", true);
  ...
  const helpText: Record<string, string> = {
    Tag: "This is the hunt code that you would enter when applying for this license. Click on the hyperlink to take you to the detailed draw stats about this code from the CPW. Click the dropdown arrow to show the harvest statistics for all units that can be hunted with this tag. The first letter specifies the species (e.g. E for Elk), the second letter specifies the sex (M, F, E for Either), the next three numbers specify the unit – however, often times there are more than a single unit associated with a given hunt code, the next number-letter pair specifies the season (O1 = Season 1, etc.), and the final letter specifies the weapon (R= Rifle, etc.).",
    ...
    Chance_at_DOL24:
      'If an applicant had the number of points specified in the "Drawn Out Level 2024" column, these are the odds they would have drawn this tag.',
    Drawn_out_level:
    ...
    Chance_at_DOL:
      'If an applicant had the number of points specified in the "Drawn Out Level 2025" column, these are the odds they would have drawn this tag.',
    slope:
      "This is the trend of points required to draw the tag over the last three draws. A green down arrow means it is becoming easier to draw (Lower points needed, or a better chance with the same number of points). A red up arrow means that it is becoming harder to draw (More points needed, or lower odds with the same number of points).",
    Chance_with_First_choice:
      "This shows your odds of drawing the tag with your number of preference points (As indicated in your profile, or in the filters section on the left) if you were to put this particular tag as your first choice in the most recent draw (2025). We cannot predict the draw results for next year, so use your best judgement when applying!",
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 h-full">
      <aside
        className={`w-full lg:w-64 bg-card p-4 rounded-lg border space-y-4 overflow-y-auto ${!showMobileFilters ? "hidden" : "block"} md:block`}
      >
        <Button
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <Label className="text-sm leading-tight">
              Show tags that don't require burning your preference points (Leftover / Choice 2-4)
            </Label>
        ...
        <div className={`space-y-2 ${showNoPointsOnly || showHybridOnly ? "opacity-50" : ""}`}>
          ...
          <input
            ...
            className="w-full"
            disabled={showNoPointsOnly || showHybridOnly}
          />
        </div>
            ...
            {[
              ...
              <div key={value} className="flex items-center space-x-2">
                <input
                  ...
                  className="rounded"
                />
                <Label htmlFor={`elk-sex-${value}`} className="cursor-pointer">
                  {label}
                </Label>
              </div>
            ))}
            ...
            {[
              ...
              <div key={value} className="flex items-center space-x-2">
                <input
                  ...
                  className="rounded"
                />
                <Label htmlFor={`elk-season-${value}`} className="cursor-pointer">
                  {label}
                </Label>
              </div>
            ))}
            ...
            {[
              ...
              <div key={value} className="flex items-center space-x-2">
                <input
                  ...
                  className="rounded"
                />
                <Label htmlFor={`elk-list-${value}`} className="cursor-pointer">
                  {label}
                </Label>
              </div>
            ))}
        ...
        <Dialog open={showHybridHelp} onOpenChange={setShowHybridHelp}>
          <DialogContent className="max-w-lg">
            ...
            <p className="text-muted-foreground leading-relaxed">
              Tags eligible for the hybrid draw are highly desirable tags, typically requiring 10 or more resident
              preference points. A small number of these tags are set aside to give those with greater than 5 points,
              but less than the required "drawn out" points a chance at drawing. Therefore, you can apply for these tags
              with your first choice, and so long as you have 5 or more preference points you will be entered into a
              random drawing that gives you a chance to draw a prized tag.
            </p>
