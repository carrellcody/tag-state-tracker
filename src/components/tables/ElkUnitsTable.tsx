...
const COLUMNS: { key: string; label: string }[] = [
  ...
  { key: "DAUAnimalDensity", label: "DAU Elk Density (Population/Acres)" },
  { key: "Bull/Cow ratio", label: "DAU Bull:Cow ratio (per 100)" },
  { key: "BullDensity", label: "DAU Bull Density (Animal Density x Bull:Cow ratio)" },
];
...
function formatCell(key: string, val: any): string {
  ...
  if (key === "DAUAnimalDensity" || key === "BullDensity") {
    const n = parseNumeric(str);
    if (!isNaN(n)) return n.toFixed(4);
  }
  if (key === "Acres" || key === "Acres Public" || key === "Population") {
    const n = parseNumeric(str);
...
export function ElkUnitsTable() {
  ...
  const [minBullCow, setMinBuckDoe] = usePersistedState("elkUnits_minBullCow", "");
  const [dauFilter, setDauFilter] = usePersistedState<string[]>("elkUnits_dauFilter", []);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [unitSearch, minPublicPercent, minBullCow, dauFilter]);
  ...
  const filtered = useMemo(() => {
    ...
    const minBD = parseNumeric(minBullCow);
    return data.filter((row: any) => {
      ...
      if (!isNaN(minBD)) {
        const bd = parseNumeric(row["Bull/Cow ratio"]);
  ...
  }, [data, unitSearch, minPublicPercent, minBullCow, dauFilter]);
  ...
  if (loading) return <div className="p-8 text-center">Loading elk unit data...</div>;
