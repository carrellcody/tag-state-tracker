...
export function OTCElkTableNew() {
  ...
  const [unitSearch, setUnitSearch] = usePersistedState('otcElkNew_unitSearch', '');
  const [showFavoritesOnly, setShowFavoritesOnly] = usePersistedState('otcElkNew_showFavoritesOnly', false);
  const [selectedSeason, setSelectedSeason] = usePersistedState('otcElkNew_selectedSeason', '');
  const [minBullCow, setMinBuckDoe] = usePersistedState('otcElkNew_minBullCow', '');
  const [minPublicLand, setMinPublicLand] = usePersistedState('otcElkNew_minPublicLand', '');
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [unitSearch, showFavoritesOnly, selectedSeason, minBullCow, minPublicLand]);
  ...
  const filteredData = useMemo(() => {
    return data.filter((row: any) => {
      ...
      if (minBullCow) {
        const bv = parseFloat(String(row['Bull/Cow ratio'] || '').replace(/[, ]/g, ''));
        if (isNaN(bv) || bv < parseFloat(minBullCow)) return false;
      }
  ...
  }, [data, unitSearch, showFavoritesOnly, favorites, selectedSeason, minBullCow, minPublicLand]);
    ...
    <div className="flex flex-col lg:flex-row gap-4 h-full">
      {(!isMobile || showMobileFilters) && (
        <aside className="w-full lg:w-64 bg-card p-4 rounded-lg border space-y-4 overflow-y-auto">
            ...
            <div className="space-y-1">
              {otcSeasons.map(season => (
                <div key={season} className="flex items-center space-x-2">
                  <input
                    type="radio"
                    id={`otcElkNew-season-${season}`}
                    name="otcElkSeason"
                    checked={selectedSeason === season}
                    onChange={() => setSelectedSeason(season)}
                    className="rounded"
                  />
                  <Label htmlFor={`otcElkNew-season-${season}`} className="cursor-pointer text-sm">{season}</Label>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2">
            <Label>Minimum Bull:Cow ratio</Label>
            <Input type="number" placeholder="Min Bull:Cow" value={minBullCow} onChange={(e) => setMinBuckDoe(e.target.value)} />
          </div>
                ...
                <tr>
                  ...
                  {visibleColumns.map((col) => (
                    <th key={col} className="relative border border-border p-2 pr-6 text-left cursor-pointer hover:bg-primary/90 text-primary-foreground" onClick={() => handleSort(col)}>
                      ...
                      {headerHelp[col] && <TableHeaderHelp label={headerLabels[col] || col} helpText={headerHelp[col]} />}
                    </th>
                  ))}
                </tr>
              </thead>
