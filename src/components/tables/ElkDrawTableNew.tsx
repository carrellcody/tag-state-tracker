...
export function ElkDrawTableNew() {
  ...
  const [hunterClassManuallyChanged, setHunterClassManuallyChanged] = usePersistedState("elkDrawNew_hunterClassManuallyChanged", false);
  const [ploFilter, setPloFilter] = usePersistedState("elkDrawNew_ploFilter", "all");
  ...
  useEffect(() => {
    const loadPreferencePoints = async () => {
      if (!user) return;
      try {
        const { data: profile, error } = await supabase.from("profiles").select("elk_preference_points, state_residency").eq("id", user.id).maybeSingle();
        if (error) { console.error("Error loading elk preference points:", error); return; }
        if (profile) {
          ...
          const sessionKey = `hunterClass_autoSet_elkNew_${user.id}`;
      ...
      } catch (error) { console.error("Error loading elk preference points:", error); }
    };
    ...
    <aside className={`w-full lg:w-64 bg-card p-4 rounded-lg border space-y-4 overflow-y-auto ${!showMobileFilters ? 'hidden' : 'block'} md:block`}>
      ...
      <div className="space-y-2">
        ...
        <RadioGroup value={hunterClass} onValueChange={(value) => { setHunterClass(value); setHunterClassManuallyChanged(true); }}>
          <div className="flex items-center space-x-2"><RadioGroupItem value="A_R" id="elkNew-class-ar" /><Label htmlFor="elkNew-class-ar">Resident Adult</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="A_NR" id="elkNew-class-anr" /><Label htmlFor="elkNew-class-anr">Non-Resident Adult</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="Y_R" id="elkNew-class-yr" /><Label htmlFor="elkNew-class-yr">Resident Youth</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="Y_NR" id="elkNew-class-ynr" /><Label htmlFor="elkNew-class-ynr">Non-Resident Youth</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="L_U" id="elkNew-class-lu" /><Label htmlFor="elkNew-class-lu">Landowner Unrestricted</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="L_R" id="elkNew-class-lr" /><Label htmlFor="elkNew-class-lr">Landowner Restricted</Label></div>
        </RadioGroup>
      </div>
        ...
        <div className="space-y-1">
          {[{ value: "All", label: "All" }, { value: "Either", label: "Either" }, { value: "Male", label: "Male" }, { value: "Female", label: "Female" }].map(({ value, label }) =>
            <div key={value} className="flex items-center space-x-2">
              <input type="checkbox" id={`elkNew-sex-${value}`} checked={sexFilter.includes(value)} onChange={e => {
              ...
              <Label htmlFor={`elkNew-sex-${value}`} className="cursor-pointer">{label}</Label>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1">
          {[{ value: "A", label: "Archery" }, { value: "M", label: "Muzzleloader" }, { value: "O1R", label: "First Rifle" }, { value: "O2R", label: "Second Rifle" }, { value: "O3R", label: "Third Rifle" }, { value: "O4R", label: "Fourth Rifle" }, { value: "E", label: "Early Rifle" }, { value: "L", label: "Late Rifle" }, { value: "Other", label: "Other" }, { value: "Any", label: "Any" }].map(({ value, label }) =>
            <div key={value} className="flex items-center space-x-2">
              <input type="checkbox" id={`elkNew-season-${value}`} checked={seasonWeapons.includes(value)} onChange={e => {
              ...
              <Label htmlFor={`elkNew-season-${value}`} className="cursor-pointer">{label}</Label>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1">
          {[{ value: "Any", label: "Any" }, { value: "A", label: "A" }, { value: "B", label: "B" }, { value: "C", label: "C" }].map(({ value, label }) =>
            <div key={value} className="flex items-center space-x-2">
              <input type="checkbox" id={`elkNew-list-${value}`} checked={listFilter.includes(value)} onChange={e => {
              ...
              <Label htmlFor={`elkNew-list-${value}`} className="cursor-pointer">{label}</Label>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <RadioGroup value={ploFilter} onValueChange={setPloFilter}>
          <div className="flex items-center space-x-2"><RadioGroupItem value="all" id="elkNew-plo-all" /><Label htmlFor="elkNew-plo-all">Show all tags</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="only" id="elkNew-plo-only" /><Label htmlFor="elkNew-plo-only">Show only PLO tags</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="none" id="elkNew-plo-none" /><Label htmlFor="elkNew-plo-none">Don't show PLO tags</Label></div>
        </RadioGroup>
      </div>
      ...
      {hunterClass !== "A_NR" && hunterClass !== "Y_NR" && <div className="space-y-2">
        ...
        <RadioGroup value={rfwFilter} onValueChange={setRfwFilter}>
          <div className="flex items-center space-x-2"><RadioGroupItem value="all" id="elkNew-rfw-all" /><Label htmlFor="elkNew-rfw-all">Show all tags</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="only" id="elkNew-rfw-only" /><Label htmlFor="elkNew-rfw-only">Show only RFW tags</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="none" id="elkNew-rfw-none" /><Label htmlFor="elkNew-rfw-none">Don't show RFW tags</Label></div>
        </RadioGroup>
      </div>}
      ...
      <div className="space-y-2">
        ...
        <RadioGroup value={showNoApplicants} onValueChange={setShowNoApplicants}>
          <div className="flex items-center space-x-2"><RadioGroupItem value="yes" id="elkNew-no-apps-yes" /><Label htmlFor="elkNew-no-apps-yes">Yes</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="no" id="elkNew-no-apps-no" /><Label htmlFor="elkNew-no-apps-no">No</Label></div>
        </RadioGroup>
      </div>
        ...
        <table className="w-full border-collapse bg-card">
          ...
          <tbody>
            {paginatedData.map((row: any, idx: number) => {
              ...
              const pdfUrl = "https://cpw.widen.net/s/lkphspbqxk/postdrawrecapreport_elk-25_05102025_1540";
