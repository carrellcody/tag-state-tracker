...
export function AntelopeDrawTable() {
  ...
  const [unitSearch, setUnitSearch] = usePersistedState('antelopeDraw_unitSearch', '');
  const [sexFilter, setSexFilter] = usePersistedState<string[]>('antelopeDraw_sexFilter', ['All']);
  const [seasonWeapons, setSeasonWeapons] = usePersistedState<string[]>('antelopeDraw_seasonWeapons', ['Any']);
  const [hunterClass, setHunterClass] = usePersistedState('antelopeDraw_hunterClass', 'A_R');
  const [hunterClassManuallyChanged, setHunterClassManuallyChanged] = usePersistedState('antelopeDraw_hunterClassManuallyChanged', false);
  const [ploFilter, setPloFilter] = usePersistedState('antelopeDraw_ploFilter', 'all');
    ...
    const loadPreferencePoints = async () => {
      ...
      try {
        const { data: profile, error } = await supabase
          .from('profiles')
          .select('antelope_preference_points, state_residency')
          .eq('id', user.id)
          .maybeSingle();
  ...
  const visibleColumns = showPreviousYears 
    ? ["Tag", "List", "Valid GMUs", "Dates", "Quota", "Drawn_out_level23", "Chance_at_DOL23", "Drawn_out_level24", "Chance_at_DOL24", "Drawn_out_level", "Chance_at_DOL", "slope", "Chance_with_First_choice", "Sex", "Weapon", "Notes"]
    : ["Tag", "List", "Valid GMUs", "Dates", "Quota", "Drawn_out_level", "Chance_at_DOL", "Chance_with_First_choice", "Sex", "Weapon", "Notes"];
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 h-full">
      <aside className={`w-full lg:w-64 bg-card p-4 rounded-lg border space-y-4 overflow-y-auto ${!showMobileFilters ? 'hidden' : 'block'} md:block`}>
            ...
            {[
              ...
              <div key={value} className="flex items-center space-x-2">
                <input
                  ...
                  className="rounded"
                />
                <Label htmlFor={`ant-list-${value}`} className="cursor-pointer">{label}</Label>
              </div>
            ))}
          </div>
        </div>
        ...
        <Button 
          onClick={() => setShowMobileFilters(false)} 
          className="w-full md:hidden shadow-[0_4px_0_0_hsl(180,30%,45%)] hover:shadow-[0_2px_0_0_hsl(180,30%,45%)] hover:translate-y-[2px] active:shadow-none active:translate-y-[4px] transition-all"
        >
          Apply filters and view data
        </Button>
      </aside>
                                  ...
                                  <tr key={unit}>
                                     <td className="border p-1 text-primary-dark">
                                       {harvestRow.onx && !isMobile ? (
                                         <a href={harvestRow.onx} target="_blank" rel="noopener noreferrer" className="hover:underline">
                                           {harvestRow.Unit}
                                         </a>
                                       ) : (
                                         harvestRow.Unit
