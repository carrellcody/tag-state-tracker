...
export function DeerDrawTableNew() {
  ...
  const [hunterClassManuallyChanged, setHunterClassManuallyChanged] = usePersistedState("deerDrawNew_hunterClassManuallyChanged", false);
  const [ploFilter, setPloFilter] = usePersistedState("deerDrawNew_ploFilter", "all");
  const [rfwFilter, setRfwFilter] = usePersistedState("deerDrawNew_rfwFilter", "all");
  const [minPoints, setMinPoints] = usePersistedState("deerDrawNew_minPoints", 0);
  const [maxPoints, setMaxPoints] = usePersistedState("deerDrawNew_maxPoints", 32);
  const [minSuccessRate, setMinSuccessRate] = usePersistedState("deerDrawNew_minSuccessRate", 0);
  const [userPreferencePoints, setUserPreferencePoints] = usePersistedState("deerDrawNew_userPreferencePoints", 0);
  const [pointsInitialized, setPointsInitialized] = usePersistedState("deerDrawNew_pointsInitialized", false);
  const [showNoApplicants, setShowNoApplicants] = usePersistedState("deerDrawNew_showNoApplicants", "no");
  ...
  useEffect(() => {
    const loadPreferencePoints = async () => {
      if (!user) return;
      try {
        const { data: profile, error } = await supabase.from("profiles").select("deer_preference_points, state_residency").eq("id", user.id).maybeSingle();
        if (error) { console.error("Error loading deer preference points:", error); return; }
        if (profile) {
          ...
          const sessionKey = `hunterClass_autoSet_deerNew_${user.id}`;
      ...
      } catch (error) { console.error("Error loading deer preference points:", error); }
    };
  ...
  const huntCodeMap = useMemo(() => {
    ...
    if (import.meta.env.DEV) {
      console.log("[DeerDrawTableNew] codePages rows:", codePages.length, "map size:", Object.keys(map).length, "first row keys:", codePages[0] ? Object.keys(codePages[0]) : []);
    }
    return map;
  }, [codePages]);
    ...
    <aside className={`w-full lg:w-64 bg-card p-4 rounded-lg border space-y-4 overflow-y-auto ${!showMobileFilters ? 'hidden' : 'block'} md:block`}>
      ...
      <div className="space-y-2">
        ...
        <RadioGroup value={hunterClass} onValueChange={(value) => { setHunterClass(value); setHunterClassManuallyChanged(true); }}>
          <div className="flex items-center space-x-2"><RadioGroupItem value="A_R" id="deerNew-class-ar" /><Label htmlFor="deerNew-class-ar">Resident Adult</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="A_NR" id="deerNew-class-anr" /><Label htmlFor="deerNew-class-anr">Non-Resident Adult</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="Y_R" id="deerNew-class-yr" /><Label htmlFor="deerNew-class-yr">Resident Youth</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="Y_NR" id="deerNew-class-ynr" /><Label htmlFor="deerNew-class-ynr">Non-Resident Youth</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="L_U" id="deerNew-class-lu" /><Label htmlFor="deerNew-class-lu">Landowner Unrestricted</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="L_R" id="deerNew-class-lr" /><Label htmlFor="deerNew-class-lr">Landowner Restricted</Label></div>
        </RadioGroup>
      </div>
        ...
        <div className="space-y-1">
          {[{ value: "All", label: "All" }, { value: "Either", label: "Either" }, { value: "Male", label: "Male" }, { value: "Female", label: "Female" }].map(({ value, label }) =>
            <div key={value} className="flex items-center space-x-2">
              <input type="checkbox" id={`deerNew-sex-${value}`} checked={sexFilter.includes(value)} onChange={e => {
              ...
              <Label htmlFor={`deerNew-sex-${value}`} className="cursor-pointer">{label}</Label>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1">
          {[{ value: "A", label: "Archery" }, { value: "M", label: "Muzzleloader" }, { value: "O1R", label: "First Rifle" }, { value: "O2R", label: "Second Rifle" }, { value: "O3R", label: "Third Rifle" }, { value: "O4R", label: "Fourth Rifle" }, { value: "E", label: "Early Rifle" }, { value: "L", label: "Late Rifle" }, { value: "Other", label: "Other" }, { value: "Any", label: "Any" }].map(({ value, label }) =>
            <div key={value} className="flex items-center space-x-2">
              <input type="checkbox" id={`deerNew-season-${value}`} checked={seasonWeapons.includes(value)} onChange={e => {
              ...
              <Label htmlFor={`deerNew-season-${value}`} className="cursor-pointer">{label}</Label>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1">
          {[{ value: "Any", label: "Any" }, { value: "A", label: "A" }, { value: "B", label: "B" }, { value: "C", label: "C" }].map(({ value, label }) =>
            <div key={value} className="flex items-center space-x-2">
              <input type="checkbox" id={`deerNew-list-${value}`} checked={listFilter.includes(value)} onChange={e => {
              ...
              <Label htmlFor={`deerNew-list-${value}`} className="cursor-pointer">{label}</Label>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <RadioGroup value={ploFilter} onValueChange={setPloFilter}>
          <div className="flex items-center space-x-2"><RadioGroupItem value="all" id="deerNew-plo-all" /><Label htmlFor="deerNew-plo-all">Show all tags</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="only" id="deerNew-plo-only" /><Label htmlFor="deerNew-plo-only">Show only PLO tags</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="none" id="deerNew-plo-none" /><Label htmlFor="deerNew-plo-none">Don't show PLO tags</Label></div>
        </RadioGroup>
      </div>
      ...
      {hunterClass !== "A_NR" && hunterClass !== "Y_NR" && <div className="space-y-2">
        ...
        <RadioGroup value={rfwFilter} onValueChange={setRfwFilter}>
          <div className="flex items-center space-x-2"><RadioGroupItem value="all" id="deerNew-rfw-all" /><Label htmlFor="deerNew-rfw-all">Show all tags</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="only" id="deerNew-rfw-only" /><Label htmlFor="deerNew-rfw-only">Show only RFW tags</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="none" id="deerNew-rfw-none" /><Label htmlFor="deerNew-rfw-none">Don't show RFW tags</Label></div>
        </RadioGroup>
      </div>}
      ...
      <div className="space-y-2">
        ...
        <RadioGroup value={showNoApplicants} onValueChange={setShowNoApplicants}>
          <div className="flex items-center space-x-2"><RadioGroupItem value="yes" id="deerNew-no-apps-yes" /><Label htmlFor="deerNew-no-apps-yes">Yes</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="no" id="deerNew-no-apps-no" /><Label htmlFor="deerNew-no-apps-no">No</Label></div>
        </RadioGroup>
      </div>
