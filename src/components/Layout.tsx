        ...
        <div className="container mx-auto px-4">
          <div className="flex h-11 items-center justify-between">
            ...
            <nav className="hidden md:flex items-center space-x-2">
              ...
              return <NavigationMenu key={menu.label}>
                    <NavigationMenuList>
                      <NavigationMenuItem>
                        <NavigationMenuTrigger className={`font-medium ${isRestricted ? "opacity-50 cursor-not-allowed" : ""}`} disabled={isRestricted}>
                          {menu.label}
                        </NavigationMenuTrigger>
                        <NavigationMenuContent>
                          <ul className="w-48 p-2 bg-popover">
                            {menu.items.map(item => <li key={item.to}>
                                <NavigationMenuLink asChild>
                                  {isRestricted ? <button onClick={handleRestrictedClick} className="block w-full text-left px-3 py-2 rounded-md hover:bg-accent transition-colors opacity-50 cursor-not-allowed">
                                      {item.label}
                                    </button> : <Link to={item.to} className="block px-3 py-2 rounded-md hover:bg-accent transition-colors">
                                      {item.label}
                                    </Link>}
                                </NavigationMenuLink>
                              </li>)}
          ...
          {/* Mobile Navigation */}
          {mobileMenuOpen && <nav className="md:hidden py-4 space-y-2 border-t border-border">
              {speciesMenus.map(menu => {
            const isRestricted = menu.type === "elk" && !hasElkAccess || menu.type === "deer" && !hasDeerAccess;
            return <div key={menu.label} className="space-y-1">
                    <div className={`px-3 py-2 font-semibold text-sm ${isRestricted ? "opacity-50" : ""}`}>
                      {menu.label}
                    </div>
                    {menu.items.map(item => isRestricted ? <button key={item.to} onClick={e => {
                setMobileMenuOpen(false);
                handleRestrictedClick(e);
              }} className="w-full">
                          <Button variant="ghost" className="w-full justify-start pl-6 opacity-50 cursor-not-allowed" size="sm">
                            {item.label}
                          </Button>
                        </button> : <Link key={item.to} to={item.to} onClick={() => setMobileMenuOpen(false)}>
                          <Button variant={isActive(item.to) ? "default" : "ghost"} className="w-full justify-start pl-6" size="sm">
                            {item.label}
                          </Button>
                        </Link>)}
                  </div>;
          })}
              {navLinks.map(link => <Link key={link.to} to={link.to} onClick={() => setMobileMenuOpen(false)}>
        ...
        <div className="container mx-auto px-4 py-[10px]">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-muted-foreground">
            ...
            <div className="flex gap-4">
              ...
              <a href="https://cpw.state.co.us" target="_blank" rel="noopener noreferrer" className="hover:text-foreground transition-colors">
